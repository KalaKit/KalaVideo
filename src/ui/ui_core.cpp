...
namespace UI
{
	void UI_Core::Initialize()
	{
		...
		ImGui_ImplWin32_Init(correctWindow);
#endif
	...
	kvec2 UI_Core::CenterWindow(const kvec2& size)
	{
		kvec2 contentSize = KalaWindow::GetWindowContentSize();

		float posX = (contentSize.x - size.x) / 2.0f;
		float posY = (contentSize.y - size.y) / 2.0f;

		return kvec2(posX, posY);
	}
